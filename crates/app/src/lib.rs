//! The web layer: every route and handler, written against [`Platform`] rather than any one
//! runtime. `crates/worker` runs this on Cloudflare; `crates/server` runs it on a machine.

// A handler's `Err` is the response to send instead, and a `Response` is a large type. Boxing
// it everywhere would cost more in noise than the copies cost in time.
...
pub use platform::{Cached, HttpRequest, HttpResponse, Instrumented, Job, Platform, QueryStats};
...
/// The clock and the log are facts about the process, not the request, so the platform installs
/// them once rather than every handler carrying them.
pub mod runtime {
    ...
    pub struct Hooks {
        pub now_ms: fn() -> i64,
        pub log: fn(&str),
        pub log_error: fn(&str),
    }

    static HOOKS: OnceLock<Hooks> = OnceLock::new();

    /// The second and later calls are ignored.
    ...
    pub fn log_error(line: &str) {
        (hooks().log_error)(line)
    }
}

pub use runtime::{log, log_error};
...
/// Everything a platform does before handing a request to the router, once per process: say
/// what this deployment is running and load the site's name and theme.
pub fn prepare<P: Platform>(p: &P) {
    startup::report_once(p, &startup::posture(p));
    startup::site_once(p);
}
...
/// not depend on the zone saying otherwise. Only fires on an absolute request URI, which is what
/// a Worker sees; a server behind a reverse proxy sees a relative one and leaves TLS to the proxy.
pub fn https_redirect(uri: &axum::http::Uri) -> Option<Response> {
...
/// A year of HSTS on every response, so a browser that has been here once never tries http.
pub fn hsts(resp: &mut Response) {
...
pub fn router<P: Platform>(p: P) -> Router {
    Router::new()
        .route("/", get(index::<P>))
        .route("/healthz", get(healthz))
        .route("/favicon.ico", get(favicon))
        .route("/theme.css", get(site_theme_css))
        .route("/api/me", get(api_me::<P>))
        .route("/t/{id}", get(thread_page::<P>))
        .route("/t/{id}/{slug}", get(thread_page_slug::<P>))
        .route("/p/{id}", get(post_permalink::<P>))
        .route("/login", get(login_form::<P>).post(login_submit::<P>))
        .route("/logout", post(logout::<P>))
        .route(
            "/register",
            get(register_form::<P>).post(register_submit::<P>),
        )
        .route(
            "/t/{id}/reply",
            get(reply_form::<P>).post(reply_submit::<P>),
        )
        .route("/t/{id}/held/{post}", get(held_notice))
        // `/s/{path}` and `/s/{path}/new` share one catch-all; the handler splits them.
        .route(
            "/s/{*path}",
            get(posting::space::<P>).post(posting::space_post::<P>),
        )
        .route("/u/{name}", get(posting::profile::<P>))
        .route(
            "/p/{id}/edit",
            get(posting::edit_form::<P>).post(posting::edit_submit::<P>),
        )
        .route("/p/{id}/delete", post(posting::delete_submit::<P>))
        // Account self-service.
        // Sign-in through a provider.
        .route(
            "/auth/finish",
            get(oauth::finish_form::<P>).post(oauth::finish_submit::<P>),
        )
        .route("/auth/{provider}", get(oauth::start::<P>))
        .route("/auth/{provider}/callback", get(oauth::callback::<P>))
        .route("/settings", get(account::settings::<P>))
        .route("/settings/email", post(account::change_email::<P>))
        .route("/settings/password", post(account::change_password::<P>))
        .route("/settings/sessions", post(account::end_other_sessions::<P>))
        .route(
            "/verify",
            get(account::verify_form::<P>).post(account::verify_submit::<P>),
        )
        .route(
            "/forgot",
            get(account::forgot_form::<P>).post(account::forgot_submit::<P>),
        )
        .route(
            "/reset",
            get(account::reset_form::<P>).post(account::reset_submit::<P>),
        )
        // Moderation. Forms are separate uncached pages for the same reason the reply form is.
        .route(
            "/p/{id}/report",
            get(report_form::<P>).post(report_submit::<P>),
        )
        .route(
            "/p/{id}/appeal",
            get(appeal_form::<P>).post(appeal_submit::<P>),
        )
        .route("/admin", get(admin::dashboard::<P>))
        .route(
            "/admin/thread/{id}",
            get(admin::thread_form::<P>).post(admin::thread_submit::<P>),
        )
        .route("/admin/post/{id}/state", post(admin::post_state::<P>))
        .route("/admin/users", get(admin::users::<P>))
        .route("/admin/user/{name}", get(admin::user::<P>))
        .route("/admin/user/{name}/state", post(admin::user_state::<P>))
        .route("/admin/user/{name}/role", post(admin::user_role::<P>))
        .route(
            "/admin/spaces",
            get(admin::spaces::<P>).post(admin::space_create::<P>),
        )
        .route(
            "/admin/space/{id}",
            get(admin::space_form_page::<P>).post(admin::space_submit::<P>),
        )
        .route("/admin/log", get(admin::log::<P>))
        .route("/mod/queue", get(mod_queue::<P>))
        .route("/mod/review/{id}", post(mod_review::<P>))
        .route("/modlog", get(modlog::<P>))
        // GET runs the read-only checks; POST adds the write checks, which mutate the thread.
        .route(
            "/__conformance",
            get(conformance::<P>).post(conformance_with_writes::<P>),
        )
        .fallback(not_found)
        .with_state(p)
}
...
async fn thread_page_slug<P: Platform>(
    state: State<P>,
...
async fn thread_page<P: Platform>(
    state: State<P>,
    headers: axum::http::HeaderMap,
...
#[cfg(feature = "password")]
#[handler]
async fn login_form<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
    ...
    if current_user(env.store(), &headers).await.is_some() {
...
#[cfg(feature = "password")]
#[handler]
async fn login_submit<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
    body: String,
) -> Response {
...
pub(crate) fn csrf_key<P: Platform>(env: &P) -> Option<csrf::CsrfKey> {
    csrf::CsrfKey::new(env.secret("CSRF_KEY")?.as_bytes()).ok()
}
...
#[cfg(not(feature = "password"))]
async fn login_submit<P: Platform>(State(_): State<P>) -> Response {
    external_auth()
}
#[cfg(not(feature = "password"))]
...
#[handler]
async fn logout<P: Platform>(State(env): State<P>, headers: axum::http::HeaderMap) -> Response {
    if let Some(raw) = cookie::get(cookie_header(&headers), cookie::SESSION) {
        if let Some(token) = SessionToken::parse(&raw) {
            // Still clear the cookie, so a failed write cannot leave a live session behind.
            let _ = env.store().delete_session(&token.hash()).await;
        }
    }
...
#[handler]
async fn api_me<P: Platform>(State(env): State<P>, headers: axum::http::HeaderMap) -> Response {
    let store = env.store();
    let no_store = [
    ...
    match current_user(store, &headers).await {
        Some(user) => {
            let body = serde_json::json!({
                "name": user.name,
                "moderator": can_moderate(&env, &user),
            });
...
#[handler]
async fn conformance<P: Platform>(state: State<P>, q: Query<ConformanceQuery>) -> Response {
    run_conformance(state, q, false).await
}

/// The full suite, write checks included; `POST` because it appends posts.
#[handler]
async fn conformance_with_writes<P: Platform>(
    state: State<P>,
    q: Query<ConformanceQuery>,
) -> Response {
    run_conformance(state, q, true).await
}

async fn run_conformance<P: Platform>(
    State(env): State<P>,
...
#[handler]
async fn post_permalink<P: Platform>(
    State(env): State<P>,
    UrlPath(id): UrlPath<String>,
) -> Response {
    ...
    let store = env.store();
    match locate::<P::Store>(store, &post_id).await {
        Ok(loc) => {
...
/// `#[handler]` satisfies axum's `Send` bound; Workers are single-threaded, so it is sound.
#[handler]
async fn render_thread<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
    id: String,
    slug: Option<String>,
    ...
    match fetch_page::<P::Store>(store, &thread_id, &page).await {
        ...
        Ok(page) => {
            ...
            if let Some(k) = &key {
                cache::put(&env, k, &html, cache::PAGE_HEADERS[0].1).await;
            }
...
pub(crate) fn error(code: StatusCode, msg: &str) -> Response {
    crate::log(&format!("notespace error {}: {}", code.as_u16(), msg));
...
#[handler]
async fn reply_form<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
    UrlPath(id): UrlPath<String>,
    ...
    let token = key.mint(&session, now_ms(), csrf::DEFAULT_LIFETIME_MS);
...
#[handler]
async fn reply_submit<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
    UrlPath(id): UrlPath<String>,
    body: String,
    ...
    let ids = match ids::generate_many(now_ms(), reply::MAX_PATH_RETRIES as usize + 1) {
        Ok(v) => v,
        Err(e) => return error(StatusCode::INTERNAL_SERVER_ERROR, &e),
    };
    let cfg = ReplyConfig {
...
#[cfg(feature = "password")]
#[handler]
async fn register_form<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
...
#[cfg(feature = "password")]
fn signup_code<P: Platform>(env: &P) -> Option<String> {
...
#[cfg(feature = "password")]
#[handler]
async fn register_submit<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
    body: String,
) -> Response {
...
#[cfg(not(feature = "password"))]
async fn register_submit<P: Platform>(State(_): State<P>) -> Response {
    external_auth()
}
...
#[handler]
async fn index<P: Platform>(State(env): State<P>) -> Response {
    let store = env.store();
...
pub(crate) fn can_moderate<P: Platform>(env: &P, user: &notespace_core::model::User) -> bool {
...
pub(crate) struct Signed<'a, S: Store> {
    pub(crate) store: &'a S,
...
pub(crate) async fn signed_in<'a, P: Platform>(
    env: &'a P,
    headers: &axum::http::HeaderMap,
    next: &str,
...
#[handler]
async fn report_form<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
    UrlPath(id): UrlPath<String>,
...
#[handler]
async fn report_submit<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
    UrlPath(id): UrlPath<String>,
    body: String,
...
#[handler]
async fn appeal_form<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
    UrlPath(id): UrlPath<String>,
...
#[handler]
async fn appeal_submit<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
    UrlPath(id): UrlPath<String>,
    body: String,
...
#[handler]
async fn mod_review<P: Platform>(
    State(env): State<P>,
    headers: axum::http::HeaderMap,
...
#[handler]
async fn modlog<P: Platform>(State(env): State<P>) -> Response {
    let store = env.store();
